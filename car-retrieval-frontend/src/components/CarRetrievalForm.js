...
const CarRetrievalForm = () => {
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => {
      // If serial_number is being changed, and it matches a car, auto-populate car_number
      if (name === "serial_number") {
        const found = parkedCars.find(
          (car) => car.serial_number?.toUpperCase() === upperValue
        );
        ...
        // If found, set car_number automatically
        return {
          ...prev,
          serial_number: upperValue,
          car_number: found ? found.vehicle_number?.toUpperCase() || "" : "",
        };
      }

      // If car_number is being changed, check for mismatch
      let newVehicleMismatch = vehicleMismatch;
      if (name === "car_number" && matchedCarInfo) {
        const enteredLast4 = upperValue.slice(-4);
        const parkedLast4 = (matchedCarInfo.vehicle_number || "")
          .slice(-4)
          .toUpperCase();
        newVehicleMismatch = enteredLast4 !== parkedLast4;
        setVehicleMismatch(newVehicleMismatch);
      }
    ...
    // Note: vehicleMismatch is now set inside the setFormData above to avoid race condition
  };
        ...
        <div className="w-full max-w-md bg-white p-6 rounded-lg shadow">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <input
              ...
              required
              autoComplete="off"
              // Optionally, you can make this field readOnly if you want to prevent user edits when a car is matched
              // readOnly={!!matchedCarInfo}
            />
