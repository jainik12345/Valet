...
//   useEffect(() => {
//     // Fetch initial car retrieval requests
//     fetchRequests();
...
//   // Fetch valets only when switching to the "valets" tab or when form toggles
//   useEffect(() => {
...
//         const valetNames = response.data.map((valet) => valet.name); // Only names
...
//       return alert("Invalid Aadhaar number. Must be 12 digits.");
//     }
...
//       alert("Valet registered successfully!");
//       setValetData({
//         name: "",
//         dob: "",
//         aadhaar: "",
...
//     } catch (err) {
//       console.error("Valet registration failed:", err);
//       alert("Failed to register valet.");
//     }
//   };
...
//       });
//       alert("Parked car recorded successfully!");
//       setParkedCarData({
//         serialNumber: "",
//         vehicleNumber: "",
//         basementNumber: "",
...
//       });
//     } catch (err) {
//       console.error("Failed to record parked car:", err);
//       alert("Error saving parked car data.");
//     }
//   };
...
//   // Fetch requests on mount and listen for new socket events
//   useEffect(() => {
//     fetchRequests();
//     socket.on("new_request", (newRequest) => {
//       setRequests((prev) => [newRequest, ...prev]);
...
//   // Fetch valets for side panel on "valets" tab or valet form toggle
//   useEffect(() => {
//     const fetchValetsForTab = async () => {
//       try {
//         const res = await fetch(`${BE_URL}/api/valets`);
...
//   // Optionally, fetch once on mount for initial load (not strictly needed)
//   // useEffect(() => { fetchValetList(); }, []);
...
//       // Refresh valet list in "parked" tab if needed
//       if (activeTab === "parked") fetchValetList();
//       // Refresh valets for "valets" tab if needed
//       if (activeTab === "valets") {
//         const res = await fetch(`${BE_URL}/api/valets`);
//         const data = await res.json();
//         setValets(data);
...
const SupervisorDashboard = () => {
  ...
  const [valetFormError, setValetFormError] = useState(""); // for showing "already exists" validation
  ...
  const handleValetSubmit = async (e) => {
    ...
    try {
      ...
      if (
        ...
        setValetFormError("Valet with this Aadhaar or Vehicle No already exists.");
      } else {
